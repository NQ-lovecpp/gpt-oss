import { RunContext, Usage, UserError, createCustomSpan, tool, } from '@openai/agents';
 ...
 * tracing is enabled.
 */
export function codexTool(options = {}) {
        ...
        execute: async (input, runContext = new RunContext(), details) => {
            ...
            if (usage) {
                const inputTokensDetails = typeof usage.cached_input_tokens === 'number'
                    ...
                    : undefined;
                runContext.usage.add(new Usage({
...
function resolveOutputSchema(option) {
    ...
    if (isZodObject(option)) {
        ...
        if (!schema) {
            throw new UserError('Codex output schema must be a Zod object that can be converted to JSON Schema.');
        }
        return schema;
    }
    if (isJsonObjectSchema(option)) {
        if (option.additionalProperties !== false) {
            throw new UserError('Codex output schema must set "additionalProperties" to false.');
        }
        return option;
    }
    ...
    if (existingThread) {
        if (threadId) {
            ...
            if (existingId && existingId !== threadId) {
                throw new UserError('Codex tool is configured with persistSession=true and already has an active thread.');
            }
        }
        ...
        for await (const event of events) {
            ...
            switch (event.type) {
                ...
                case 'turn.completed':
                    ...
                    break;
                case 'turn.failed':
                    throw new UserError(`Codex turn failed${event.error?.message ? `: ${event.error.message}` : ''}`);
                case 'error':
...
function buildMcpToolSpanData(item) {
    ...
    if (item.result) {
        ...
        if (typeof item.result.structured_content !== 'undefined') {
            applyTruncatedField(resultSummary, 'structured_content', toSmartString(item.result.structured_content), { maxLength: MAX_SPAN_TEXT_LENGTH, mode: 'head' });
        }
