...
function itemsToLanguageV2Messages(model, items, modelSettings) {
    ...
    for (const item of items) {
                    ...
                    content: typeof content === 'string'
                        ...
                        : content.map((c) => {
                            ...
                            if (c.type === 'input_image') {
                                const imageSource = typeof c.image === 'string'
                                    ...
                                    : typeof c.imageUrl === 'string'
                                        ...
                                        : undefined;
                                if (!imageSource) {
                                    throw new agents_1.UserError('Only image URLs are supported for user inputs.');
                                }
                            ...
                            if (c.type === 'input_file') {
                                throw new agents_1.UserError('File inputs are not supported.');
                            }
        ...
        if (item.type === 'shell_call') {
            throw new agents_1.UserError('Shell calls are not supported');
        }
        if (item.type === 'shell_call_output') {
            throw new agents_1.UserError('Shell call results are not supported');
        }
        if (item.type === 'apply_patch_call') {
            throw new agents_1.UserError('Apply patch calls are not supported');
        }
        if (item.type === 'apply_patch_call_output') {
            throw new agents_1.UserError('Apply patch call results are not supported');
        }
...
function convertLegacyToolOutputContent(output) {
    ...
    throw new agents_1.UserError(`Unsupported tool output type: ${JSON.stringify(output)}`);
}
